...
public class SqlStorage implements Storage {
    ...
    public void update(Resume resume) {
        sqlExecutor.transactionalExecute(connection -> {
            ...
            deleteContent(resume.getUuid(), "contacts");
            insertContacts(connection, resume);
            deleteContent(resume.getUuid(), "sections");
            insertSections(connection, resume);
            return null;
        });
    }
...
//        });
    ...
    private void deleteContent(String uuid, String tableName) {
        sqlExecutor.execute("DELETE FROM " + tableName + "  WHERE resume_uuid=?", statement -> {
            statement.setString(1, uuid);
            return null;
        });
    }
            ...
            for (Map.Entry<SectionType, AbstractSection> entry : resume.getSections().entrySet()) {
                ...
                switch (type) {
                    case PERSONAL, OBJECTIVE -> {
                        statement.setString(2, ((TextSection) entry.getValue()).getContent());
                    }
                    case ACHIEVEMENT, QUALIFICATIONS -> {
