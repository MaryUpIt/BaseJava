...
<section>
    <form method="post" action="resume" enctype="application/x-www-form-urlencoded">
        ...
        <c:forEach var="type" items="<%=ContactType.values()%>">
            <dl>
                <dt>${type.title}</dt>
                <dd><input ${HtmlUtil.contactPattern(type)} name="${type.name()}" size="30" value="${resume.getContact(type)}"></dd>
            </dl>
        </c:forEach>
        <h3>SECTIONS:</h3>
                ...
                <c:when test="${type=='EDUCATION' || type=='EXPERIENCE'}">
                    <c:forEach var="organization" items="<%=((OrganizationSection)section).getContent()%>"
                        ...
                        <dl>
                            ...
                            <dd><input type="text" name="${type}url" size="55" value="${organization.website}"></dd>
                        </dl>
                        <c:forEach var="period" items="${organization.periods}">
                            ...
                            <dl>
                                <dt>POSITION</dt>
                                <dd><input type="text" name="${type}${counter.index}position" size="55" value="${period.position}"></dd>
                            </dl>
                            <dl>
                                <dt>RESPONSIBILITIES</dt>
                                <dd><textarea type="text" name="${type}${counter.index}description" cols="55" rows="5">${period.responsibilities}</textarea>
                                </dd>
                            </dl>
                        </c:forEach>
                    </c:forEach>
